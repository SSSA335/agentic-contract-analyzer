...
- Locally, every step in both `api` and `worker` is traced with OpenTelemetry and visualized in Jaeger — this tracing is what surfaced a cold-start bottleneck adding 80%+ latency to one pipeline stage, which was then fixed.
